...
package GEM.Random_Descent is
   ...
   type LF_Array is
     array
       (Positive range <>) of Long_Float; -- this could be in generic formal
   ...
   procedure Markov
     (Set : in out LF_Array;  -- Randomly selects from param set
      Ref : out LF_Array;     -- returns the unchanged set
      Spread : in Long_Float;
      Cal : in LF_Array);     -- Calibrated starting point

   procedure Markov
     (Value : in out Long_Float;  -- Randomly selects from param set
