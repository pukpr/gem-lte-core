...
def run_round_robin(
    ...
    for round_num in range(1, rounds + 1):
        ...
        for idx in indices:
            ...
            multi_dir = base_dir / "multi_index"

            # Priority: multi_index shared → round N-1 → lpap_top.coord.p → ir_ltep.coord.p → lt.exe.p
            candidates = [
                (multi_dir / "shared_params.p", "multi_index/shared_params.p"),
                (run_dir / f"lt.exe.pysr.lpap_top.round{round_num-1}.p",
                 f"{idx}/lt.exe.pysr.lpap_top.round{round_num-1}.p"),
                (run_dir / "lt.exe.pysr.lpap_top.coord.p",
                 f"{idx}/lt.exe.pysr.lpap_top.coord.p"),
                (run_dir / "lt.exe.pysr.ir_ltep.coord.p",
                 f"{idx}/lt.exe.pysr.ir_ltep.coord.p"),
                (run_dir / "lt.exe.p", f"{idx}/lt.exe.p"),
            ]
            for cand_path, cand_name in candidates:
                if cand_path.exists():
                    params = json.loads(cand_path.read_text())
                    loaded_from = cand_name
                    # If loading from multi_index shared params, restore per-index ltep/IR/harm
                    if "multi_index" in loaded_from and "ltep" not in params:
                        orig_path = run_dir / "lt.exe.p"
                        if orig_path.exists():
                            orig = json.loads(orig_path.read_text())
                            params["ltep"] = orig["ltep"]
                            params["IR"] = orig.get("IR", 0.0)
                            if "harm" in orig:
                                params["harm"] = orig["harm"]
                    break
