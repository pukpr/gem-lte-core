#!/usr/bin/env python3
"""lte_forward_million.py — lte_forward.py, extended to run the forward
pipeline over a millennial date range instead of just the observed record.
...
---------------------
The observed .dat series starts at dates[0] (e.g. 1880.0 for amo.dat).
IDATE (src/gem-lte-primitives.adb IIR) seeds the IIR integrator at the first
sample with Date > IDATE, integrates forward from there, and reconstructs
"pre-history" backward down to index 0. When dates[0] IS the IDATE seed
(the normal case), that backward pass is a single-step stub — it never
really exercises the reconstruction.

extend_to_start_year() prepends synthetic monthly dates from START_YEAR
(default 1200.0) up to dates[0], padded with the first real observation
(not zero, so Filter9Point/median smoothing doesn't distort the real
boundary sample). This gives the IIR backward pass genuine multi-century
pre-history to reconstruct when IDATE stays at its normal seed (e.g. 1880):
the backward loop now runs for real, all the way from 1880 down to 1200.
The least-squares training window is clamped to never dip into the
synthetic pre-history (real_start_idx), so the regression fit — and hence
`model` from real_start_idx onward — is unaffected by the extension itself.

The fit's secular part (level + k0*forcing + trend*date + accel*(date-date0)^2)
is only meaningful over the 1880-2022 window it was fit to: `level` alone
runs ~+219 there, cancelled almost entirely by trend*date over that narrow
span, but extrapolated 700 years back to 1200AD that cancellation breaks
down into a large drifting DC offset. So the saved output (model_save,
written to the CSV and used for --verify) drops level/k0/trend/accel
entirely and keeps only the oscillatory LTE (amp/phase) + annual/
semi-annual terms, which are ~zero-mean by construction over the whole
1200-present interval — printed each run as a sanity check. `model` (with
the full secular fit) is kept internally for the CC metric, which compares
against `data`'s own real trend over the real-data window.

--verify-idate runs the pipeline twice on the identical extended date
range, once seeded at IDATE=1880 (exercising the backward reconstruction
down to 1200) and once seeded at IDATE=1200 (forward-only, same `init`
parameter, since 1200 sits at/before dates[0] so idx clamps to 0). It
reports how closely the two runs' `model`/`forcing` agree over the real
data window (1880-present) — NOTE: the IIR backward step (line ~201) is
NOT the algebraic inverse of the forward step (it uses raw[i-1]/sign(y[i])
instead of raw[i]/sign(y[i-1])), so this is an empirical check, not a
mathematical identity — read the printed diffs rather than assuming a
match.

Usage
-----
    ./lte_forward_million.py amo --verify
    ./lte_forward_million.py amo --verify-idate
    ./lte_forward_million.py amo --start-year 1200 --idate 1880
...
def lte_response(fv, dates, m, amp, phase, level, k0, trend, accel,
                 nonlin, annual, third=0.0, date0=None):
    ...
    date_first (`date0`) defaults to dates[0] (lte_forward.py behavior); the
    million-year extension passes the real observation start explicitly,
    since dates[0] there is the synthetic 1200AD anchor, not date_first."""
    if date0 is None:
        date0 = dates[0]
...
# ---------------------------------------------------------------------------
# Millennial date-range extension (not part of the Ada reference — the .dat
# files only cover the observed record)
# ---------------------------------------------------------------------------

def extend_to_start_year(dates: np.ndarray, data: np.ndarray, start_year: float):
    """Prepend synthetic monthly samples from `start_year` up to dates[0],
    so the IIR integrator (see `iir`) gets genuine multi-century pre-history
    to backward-reconstruct instead of the one-step stub it gets when its
    IDATE seed sits at dates[0]. Padding repeats the first real observation
    (constant, not zero) so Filter9Point/median smoothing does not distort
    the real boundary sample. Returns (dates, data, real_start_idx) where
    real_start_idx is the index of the first genuine observation."""
    dt = dates[1] - dates[0]
    n_prepend = int(round((dates[0] - start_year) / dt))
    if n_prepend <= 0:
        return dates, data, 0
    pre_dates = dates[0] - dt * np.arange(n_prepend, 0, -1)
    pre_data = np.full(n_prepend, data[0])
    return (np.concatenate([pre_dates, dates]),
            np.concatenate([pre_data, data]), n_prepend)
...
def forward(cfg: Config, params: dict):
    ...
    start_year = cfg.get("START_YEAR", 1200.0)
    dates, data, real_start_idx = extend_to_start_year(dates, data, start_year)
    ...
    # Clamped to real_start_idx so the synthetic pre-history padding (see
    # extend_to_start_year) never enters the regression target `data[sel]`.
    if exclude:
        sel = np.r_[real_start_idx:first + 1, last:n]
    else:
        sel = np.arange(max(first, real_start_idx), last + 1)
    ...
    model = lte_response(forcing, dates, m, amp, phase, level, k0,
                         ...
                         date0=dates[real_start_idx])
    # `level` is only meaningful alongside trend*date: over the 1880-2022
    # fit window trend*date is comparable in magnitude to level and the two
    # cancel (that's why `level` alone can be ~+219 while the real AMO
    # series is O(1)-O(5)); trend/accel are otherwise fit only to that
    # narrow window and, extrapolated linearly/quadratically 700 years back
    # to 1200AD, swing wildly. So level+k0*F+trend*date+accel*(date-date0)^2
    # all have to go together, not just trend/accel — dropping only
    # trend/accel would strand the ~219 `level` term as a huge, un-cancelled
    # DC offset over the whole range. model_save keeps just the oscillatory
    # LTE (amp/phase) + annual/semi-annual terms, which are ~zero mean over
    # the full 1200-present interval by construction. `model` (with the
    # full secular fit) is kept for the CC metric/--verify, which must
    # match data's own trend over the real-data window.
    model_save = lte_response(forcing, dates, m, amp, phase, 0.0, 0.0,
                              0.0, 0.0, nonlin, annual,
                              date0=dates[real_start_idx])
    if cfg.get("SECULAR", True):
        trend, accel = abs(trend), abs(accel)
    if B["IR"] != 0.0:
        model = model.copy()
        model_save = model_save.copy()
        for i in range(n - 1, 11, -1):
            model[i] -= B["IR"] * model[i - 12]
            model_save[i] -= B["IR"] * model_save[i - 12]

    # 9) smoothing
    if f9 > 0:
        for _ in range(f9):
            model = filter9point(model)
            model_save = filter9point(model_save)
    else:
        model = median3(model)
        model = fir(fir(model, filt, 1.0 - 2.0 * filt, filt),
                    filt, 1.0 - 2.0 * filt, filt)
        model_save = median3(model_save)
        model_save = fir(fir(model_save, filt, 1.0 - 2.0 * filt, filt),
                         filt, 1.0 - 2.0 * filt, filt)
    ...
    mspec = window_box(me_power_spectrum(forcing, model_save,
                                         f_start, f_step, n), 2)
    dspec = window_box(me_power_spectrum(forcing, data,
                                         f_start, f_step, n), 2)
    return dict(dates=dates, model=model, model_save=model_save, data=data,
                forcing=forcing, fspec=fspec, mspec=mspec, dspec=dspec,
                cc=corr, periods=periods, m=m, nm=nm, nh=len(harms), f9=f9,
                real_start_idx=real_start_idx, idate=idate,
...
def write_results(path: Path, r: dict):
    ...
    Writes model_save (level/k0/trend/accel — the whole secular fit —
    removed, oscillatory LTE+annual terms only) when present, so the
    millennial extrapolation of that secular fit doesn't leave a large
    spurious DC offset near 1200AD; falls back to `model` for parity with
    lte_forward.py output."""
    model_col = r.get("model_save", r["model"])
    with open(path, "w") as f:
        for i in range(len(r["dates"])):
            row = [r["dates"][i], model_col[i], r["data"][i],
...
def verify(expected: Path, r: dict) -> bool:
    ...
    restricted to date/model/data/forcing over the real-data window
    (from real_start_idx on). spec_freq/spec_model/spec_data are computed
    over the full millennial-extended series (window_box + me_power_spectrum
    integrate from index 8 of the *extended* array), so they are not
    comparable to the observed-record-only reference and are skipped.
    ...
    i0 = r.get("real_start_idx", 0)
    got = np.column_stack([r["dates"], r["model"], r["data"],
                           r["forcing"]])[i0:]
    if exp.shape[0] != got.shape[0]:
        print(f"row count mismatch: {exp.shape[0]} vs {got.shape[0]}")
        return False
    names = ["date", "model", "data", "forcing"]
    ok = True
    ...
    print("(spec_freq/spec_model/spec_data skipped: recomputed over the "
          "full extended series, not comparable to the observed-record ref)")
    print(f"\nCC(train) = {r['cc']:.6f}")
    return ok


def compare_idate(cfg_overrides_a: dict, cfg_overrides_b: dict, resp: dict,
                   params: dict, real_start_idx_key="real_start_idx") -> bool:
    """Run forward() twice on the identical (extended) date range, once per
    override dict (each supplying its own IDATE), and report how closely
    model/forcing agree over the real-data window. See the module docstring
    ("Millennial extension") for why this is an empirical check, not a
    guaranteed identity: the IIR backward step is not the algebraic inverse
    of the forward step."""
    ra = forward(Config(resp, cfg_overrides_a), params)
    rb = forward(Config(resp, cfg_overrides_b), params)
    i0 = max(ra["real_start_idx"], rb["real_start_idx"])
    print(f"IDATE={ra['idate']:.1f} vs IDATE={rb['idate']:.1f}  "
          f"(comparing dates[{i0}:], i.e. the real-data window)")
    print(f"{'column':<10}{'max|diff|':>14}{'rel':>12}{'corr':>14}   status")
    ok = True
    for name in ("model", "forcing"):
        a, b = ra[name][i0:], rb[name][i0:]
        d = float(np.max(np.abs(a - b)))
        scale = max(float(np.max(np.abs(a))), 1e-30)
        rel = d / scale
        corr = float(np.corrcoef(a, b)[0, 1]) if np.std(b) > 0 else 1.0
        good = rel <= 1e-5 and corr > 1.0 - 1e-9
        ok &= good
        print(f"{name:<10}{d:>14.6e}{rel:>12.2e}{corr:>14.9f}"
              f"   {'MATCH' if good else 'DIFF'}")
    return ok
...
def main() -> int:
    ...
    ap.add_argument("--out", default="lte_forward_million_results.csv")
    ap.add_argument("--verify", action="store_true",
                    help="compare against lte_results.csv in the index dir "
                         "(real-data window only; see verify() docstring)")
    ap.add_argument("--start-year", type=float, default=1200.0,
                    help="prepend synthetic dates back to this year "
                         "(default: 1200.0)")
    ap.add_argument("--idate", type=float, default=None,
                    help="override IDATE (default: resp/env value, "
                         "normally 1880.0)")
    ap.add_argument("--verify-idate", action="store_true",
                    help="run twice (IDATE=1880 vs IDATE=1200) on the same "
                         "extended range and report model/forcing agreement "
                         "over the real-data window, instead of a single run")
    args = ap.parse_args()
    ...
    overrides["START_YEAR"] = str(args.start_year)
    if args.idate is not None:
        overrides["IDATE"] = str(args.idate)

    params = json.loads(Path(args.param or run_dir / "lt.exe.p").read_text())

    if args.verify_idate:
        ov_1880 = dict(overrides, IDATE="1880.0")
        ov_1200 = dict(overrides, IDATE=str(args.start_year))
        ok = compare_idate(ov_1880, ov_1200, resp, params)
    ...
    print(f"forward pass complete: {run_dir / args.out}  "
          ...
          f"F9={r['f9']}, START_YEAR={args.start_year}, IDATE={r['idate']})")
    reg = r["reg"]
    ...
    print(f"saved-model mean (secular fit removed: level/k0/trend/accel) over "
          f"[{r['dates'][0]:.1f}, {r['dates'][-1]:.1f}] = "
          f"{np.mean(r['model_save']):.6f}  (check: should be ~0)")
